##############################################################################################################
#
# FortiManager - High Availability Deployment
# Terraform deployment template for AWS
#
##############################################################################################################
# locals
##############################################################################################################
locals {
  fmg1_name = "${var.prefix}-fmg1"
  fmg1_vars = {
    fmg_vm_name           = local.fmg1_name
    fmg_admin_port        = var.fmg_admin_port
    fmg_license_file      = var.fmg1_byol_license_file
    fmg_license_fortiflex = var.fmg1_byol_fortiflex_license_token
    peer_ipaddr           = aws_network_interface.fmg2.private_ip
    peer_serial_number    = var.fmg2_byol_serial_number
    ha_priority           = 100
    ha_password           = var.ha_password
    ha_ipaddr             = var.ha_ip == "public" ? aws_eip.vip[0].public_ip : cidrhost(data.aws_subnet.fmg1.cidr_block, 100)
  }

  fmg2_name = "${var.prefix}-fmg2"
  fmg2_vars = {
    fmg_vm_name           = local.fmg2_name
    fmg_admin_port        = var.fmg_admin_port
    fmg_license_file      = var.fmg2_byol_license_file
    fmg_license_fortiflex = var.fmg2_byol_fortiflex_license_token
    peer_ipaddr           = aws_network_interface.fmg1.private_ip
    peer_serial_number    = var.fmg1_byol_serial_number
    ha_priority           = 1
    ha_password           = var.ha_password
    ha_ipaddr             = var.ha_ip == "public" ? aws_eip.vip[0].public_ip : cidrhost(data.aws_subnet.fmg1.cidr_block, 100)
  }

  # AMI ID selection based on license type
  ami_id = var.fmg_license_type == "byol" ? (
    length(data.aws_ami.fortimanager_byol) > 0 ? data.aws_ami.fortimanager_byol[0].id : null
    ) : (
    length(data.aws_ami.fortimanager_payg) > 0 ? data.aws_ami.fortimanager_payg[0].id : null
  )

  # User data for initialization
  fmg1_user_data = base64encode(templatefile("${path.module}/templates/user_data.tpl", local.fmg1_vars))
  fmg2_user_data = base64encode(templatefile("${path.module}/templates/user_data.tpl", local.fmg2_vars))

  # Security group rules
  management_ingress_rules = [
    {
      from_port   = 0
      to_port     = 0
      protocol    = "22"
      cidr_blocks = ["0.0.0.0/0"]
      description = "FMG HA"
    },
    {
      from_port   = 22
      to_port     = 22
      protocol    = "tcp"
      cidr_blocks = ["0.0.0.0/0"]
      description = "SSH access"
    },
    {
      from_port   = 443
      to_port     = 443
      protocol    = "tcp"
      cidr_blocks = var.admin_cidr
      description = "HTTPS management access"
    },
    {
      from_port   = 541
      to_port     = 541
      protocol    = "tcp"
      cidr_blocks = var.fortigate_cidr
      description = "FortiGate to FortiManager secure log transmission"
    },
    {
      from_port   = 514
      to_port     = 514
      protocol    = "udp"
      cidr_blocks = var.fortigate_cidr
      description = "Syslog reception"
    },
    {
      from_port   = 5199
      to_port     = 5199
      protocol    = "tcp"
      cidr_blocks = ["0.0.0.0/0"]
      description = "FMG HA"
    }
  ]

  fmg1_tags = merge(var.fortinet_tags, {
    Name = local.fmg1_name
  })
  fmg2_tags = merge(var.fortinet_tags, {
    Name = local.fmg1_name
  })
}

##############################################################################################################
# Security group for FortiManager
##############################################################################################################
resource "aws_security_group" "fortimanager" {
  name_prefix = "${var.prefix}-sg"
  description = "Security group for FortiManager ${var.prefix}"
  vpc_id      = var.vpc_id

  # Dynamic ingress rules
  dynamic "ingress" {
    for_each = local.management_ingress_rules
    content {
      from_port   = ingress.value.from_port
      to_port     = ingress.value.to_port
      protocol    = ingress.value.protocol
      cidr_blocks = ingress.value.cidr_blocks
      description = ingress.value.description
    }
  }

  # Allow all outbound traffic
  egress {
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
    description = "All outbound traffic"
  }

  tags = merge(var.fortinet_tags, {
    Name = "${var.prefix}-security-group"
  })
}

##############################################################################################################
# Elastic IPs
##############################################################################################################
resource "aws_eip" "fmg1" {
  count = var.ha_ip == "public" ? 1 : 0

  domain            = "vpc"
  network_interface = aws_network_interface.fmg1.id

  depends_on = [aws_instance.fmg1]

  tags = merge(var.fortinet_tags, {
    Name = "${var.prefix}-fmg1-eip"
  })
}

resource "aws_eip" "fmg2" {
  count = var.ha_ip == "public" ? 1 : 0

  domain            = "vpc"
  network_interface = aws_network_interface.fmg2.id

  depends_on = [aws_instance.fmg2]

  tags = merge(var.fortinet_tags, {
    Name = "${var.prefix}-fmg2-eip"
  })
}

resource "aws_eip" "vip" {
  count = var.ha_ip == "public" ? 1 : 0

  domain            = "vpc"

  tags = merge(var.fortinet_tags, {
    Name = "${var.prefix}-eip"
  })
}

##############################################################################################################
# EC2 Instances
##############################################################################################################
# Network interface for FortiManager
resource "aws_network_interface" "fmg1" {
  subnet_id       = var.subnet_ids[0]
  security_groups = [aws_security_group.fortimanager.id]
  private_ip_list_enabled = true
  private_ip_list = var.ha_ip == "public" ? [] : [cidrhost(data.aws_subnet.fmg1.cidr_block, 50),local.fmg1_vars.ha_ipaddr]

  tags = merge(var.fortinet_tags, {
    Name = "${local.fmg1_name}-nic1"
  })
}

# Additional EBS volume for log storage
resource "aws_ebs_volume" "fmg1_logs" {
  count = var.enable_log_volume ? 1 : 0

  availability_zone = var.subnet_availability_zones[0]
  size              = var.fmg_log_volume_size
  type              = var.fmg_log_volume_type
  encrypted         = true

  tags = merge(local.fmg1_tags, {
    Name = "${var.prefix}-log-volume"
  })
}

# Attach log volume to instance
resource "aws_volume_attachment" "fmg1_logs" {
  count = var.enable_log_volume ? 1 : 0

  device_name = "/dev/sdf"
  volume_id   = aws_ebs_volume.fmg1_logs[0].id
  instance_id = aws_instance.fmg1.id
}

# FortiManager EC2 instance
resource "aws_instance" "fmg1" {
  ami           = local.ami_id
  instance_type = var.fmg_vmsize
  key_name      = var.key_name

  availability_zone = var.subnet_availability_zones[0]

  # Network configuration
  primary_network_interface {
    network_interface_id = aws_network_interface.fmg1.id
  }

  # Root volume configuration
  root_block_device {
    volume_type           = "gp2"
    volume_size           = var.fmg_root_volume_size
    encrypted             = true
    delete_on_termination = true

    tags = merge(var.fortinet_tags, {
      Name = "${var.prefix}-root-volume"
    })
  }

  # IAM instance profile
  iam_instance_profile = var.create_iam_role ? aws_iam_instance_profile.fortimanager[0].name : null

  # User data for initial configuration
  user_data_base64 = local.fmg1_user_data

  # Monitoring
  monitoring = var.enable_detailed_monitoring

  # Termination protection
  disable_api_termination = var.enable_termination_protection

  # Instance metadata options
  metadata_options {
    http_endpoint = "enabled"
    http_tokens   = "optional"
  }

  tags = merge(local.fmg1_tags, {
    Name = local.fmg1_name
  })

  lifecycle {
    ignore_changes = [
      ami,
      user_data
    ]
  }
}

resource "aws_network_interface" "fmg2" {
  subnet_id       =  var.ha_ip == "public" ? var.subnet_ids[1] : var.subnet_ids[0]
  security_groups = [aws_security_group.fortimanager.id]

  tags = merge(var.fortinet_tags, {
    Name = "${local.fmg2_name}-nic1"
  })
}

# Additional EBS volume for log storage
resource "aws_ebs_volume" "fmg2_logs" {
  count = var.enable_log_volume ? 1 : 0

  availability_zone = var.ha_ip == "public"? var.subnet_availability_zones[1] : var.subnet_availability_zones[0]
  size              = var.fmg_log_volume_size
  type              = var.fmg_log_volume_type
  encrypted         = true

  tags = merge(local.fmg2_tags, {
    Name = "${var.prefix}-log-volume"
  })
}

# Attach log volume to instance
resource "aws_volume_attachment" "fmg2_logs" {
  count = var.enable_log_volume ? 1 : 0

  device_name = "/dev/sdf"
  volume_id   = aws_ebs_volume.fmg2_logs[0].id
  instance_id = aws_instance.fmg2.id
}

resource "aws_instance" "fmg2" {
  ami           = local.ami_id
  instance_type = var.fmg_vmsize
  key_name      = var.key_name

  availability_zone = var.ha_ip == "public" ? var.subnet_availability_zones[1] : var.subnet_availability_zones[0]

  # Network configuration
  primary_network_interface {
    network_interface_id = aws_network_interface.fmg2.id
  }

  # Root volume configuration
  root_block_device {
    volume_type           = "gp2"
    volume_size           = var.fmg_root_volume_size
    encrypted             = true
    delete_on_termination = true

    tags = merge(var.fortinet_tags, {
      Name = "${var.prefix}-root-volume"
    })
  }

  # IAM instance profile
  iam_instance_profile = var.create_iam_role ? aws_iam_instance_profile.fortimanager[0].name : null

  # User data for initial configuration
  user_data_base64 = local.fmg2_user_data

  # Monitoring
  monitoring = var.enable_detailed_monitoring

  # Termination protection
  disable_api_termination = var.enable_termination_protection

  # Instance metadata options
  metadata_options {
    http_endpoint = "enabled"
    http_tokens   = "optional"
  }

  tags = merge(local.fmg2_tags, {
    Name = local.fmg2_name
  })

  lifecycle {
    ignore_changes = [
      ami,
      user_data
    ]
  }
}

##############################################################################################################
# IAM
##############################################################################################################
# IAM role for FortiManager
resource "aws_iam_role" "fortimanager" {
  count = var.create_iam_role ? 1 : 0

  name = "${var.prefix}-fmg-role"
  path = "/"

  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        Action = "sts:AssumeRole"
        Effect = "Allow"
        Sid    = ""
        Principal = {
          Service = "ec2.amazonaws.com"
        }
      },
    ]
  })

  tags = merge(var.fortinet_tags, {
    Name = "${var.prefix}-fmg-role"
  })
}

# IAM instance profile
resource "aws_iam_instance_profile" "fortimanager" {
  count = var.create_iam_role ? 1 : 0

  name = "${var.prefix}-fmg-instance-profile"
  role = aws_iam_role.fortimanager[0].name

  tags = merge(var.fortinet_tags, {
    Name = "${var.prefix}-fmg-instance-profile"
  })
}

# IAM policy for FortiManager
resource "aws_iam_role_policy" "fortimanager" {
  count = var.create_iam_role ? 1 : 0

  name = "${var.prefix}-fmg-policy"
  role = aws_iam_role.fortimanager[0].id

  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        Effect = "Allow"
        Action = [
          "ec2:AssignPrivateIpAddresses",
          "ec2:DescribeSubnets",
          "ec2:DescribeNetworkInterfaces",
          "ec2:DescribeAddresses",
          "ec2:AssociateAddress",
          "ec2:CreateTags",
          "s3:GetObject"
        ]
        Resource = "*"
      }
    ]
  })
}
